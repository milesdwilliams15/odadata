...
#' @return `add_oda()` returns either a state-year or dyad-year data frame populated with bilateral official development assistance (ODA) commitments and disbursements in millions of 2023 US Dollars. All zero values are true zeros and all `NA` values are truly invalid. ODA data runs from 1960 to 2023 for disbursements and from 1966 to 2023 for commitments. If the supplied data object is a dyad-year data frame, ODA values are totals for a donor-recipient pair from state 1 to state 2. If the supplied data is a state-year data frame, ODA values are total bilateral ODA for either a donor (if `type = "donor"`) or a recipient (if `type = "recipient"`).
